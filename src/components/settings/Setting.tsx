...
import {
	...
	SettingContext,
	SettingSlotContext,
} from 'src/contexts/setting-context'
...
export default function Setting({
	...
	disabled = false,
	heading = false,
	slots: { info: infoSlot, name: nameSlot, desc: descSlot, control: controlSlot } = {},
	tooltip = '',
}: {
	children?: ReactNode
	...
	const settingSlotsContext = useMemo(
		() =>
			...
			}) satisfies Record<string, ContextType<typeof SettingSlotContext>>,
		[setting]
	)
	return (
...
Setting.Dropdown = function Dropdown({
	...
	return !dropdown ? undefined : <>{createPortal(children, dropdown.selectEl)}</>
}
...
Setting.ExtraButton = function ExtraButton({
	...
	return !extraButton ? undefined : <>{createPortal(children, extraButton.extraSettingsEl)}</>
}
...
Setting.MomentFormat = function MomentFormat({
	...
	return !momentFormat ? undefined : <>{createPortal(children, momentFormat.inputEl)}</>
}
